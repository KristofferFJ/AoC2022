...
fun main() {
    ...
    repeat(26) {
        val manPaths = mutableListOf(mutableListOf<Action>())
        createRoutes(startPath.toMutableList(), startPath.size, manPaths)
        val top100ManPaths = manPaths.sortedBy { it.pressure() }.reversed().subList(0, 100)
        val elephantPaths = top100ManPaths.toMutableList()
        val combinedPaths: List<Pair<List<Action>, List<Action>>> = top100ManPaths.flatMap {
                manPath -> elephantPaths.map { Pair(manPath, it) }
        }
        val bestPaths = combinedPaths.sortedBy { it.pressure() }.reversed().filter { it.first.size > 0 && it.second.size > 0}
...
fun createRoutes(path: MutableList<Action>, startActions: Int, paths: MutableList<MutableList<Action>>) {
    if (path.size == startActions + 10 || path.size == 26) {
        paths.add(path)
        return
    }
...
fun Pair<List<Action>, List<Action>>.pressure(): Int {
    val valveStatus = valves.associate { it.name to false }.toMutableMap()
    return this.first.mapIndexed { index, actions ->
        when (actions) {
            is TurnOn -> {
                val name = (this.first[index - 1] as Valve).name
                if(valveStatus[name]!!) {
                    return 0
                }
                valveStatus[name] = true
                return (this.first[index - 1] as Valve).flowRate * (26 - index)
            }
            else -> 0
        }
    }.sum()
