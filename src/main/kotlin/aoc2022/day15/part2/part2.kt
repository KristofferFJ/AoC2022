...
const val maxSize = 400000
data class Point(val x: Int, val y: Int)
data class SensorBeacon(val sensor: Point, val beacon: Point) {
    fun getCoveredPointsOnLine(line: Int): Set<Int> {
        ...
        if(xMax < 1) return emptySet()
        return IntRange(maxOf(-xMax + sensor.x, 0), minOf(xMax + sensor.x, maxSize)).toSet()
    }
}

fun getAvailablePointsOnLine(sensorBeacons: List<SensorBeacon>, y: Int): Set<Int> {
    val availableValues = IntRange(0, maxSize).toMutableSet()
    sensorBeacons.forEach {
        availableValues.removeAll(it.getCoveredPointsOnLine(y))
        if(availableValues.isEmpty()) return emptySet()
    }
...
fun main() {
    ...
    IntRange(0, maxSize).forEach {
        if(getAvailablePointsOnLine(sensorBeacons, it).isNotEmpty()){
            println(it)
            println(getAvailablePointsOnLine(sensorBeacons, it))
            if(it % 10000 == 0) {
                println(it)
            }
        }
